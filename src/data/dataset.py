...
class StrokeDataset(Dataset):
    ...
    def __init__(
        self, data_dir: str, patient_list: list, mode: str = "2d", transform=None
    ):
        ...
        print(
            f"Dataset initialized in {self.mode.upper()} mode. Total samples: {len(self)}"
        )
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.mode in ["2d", "2dr"]:
            ...
            item = {
                ...
                "label": torch.from_numpy(mask),
                # Storing metadata for 2D->3D reconstruction later
                "meta": {"patient_id": sample_info["patient_id"]},
            }
